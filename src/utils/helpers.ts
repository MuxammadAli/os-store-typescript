...
export const calculateTokenExpiration = (
    ...
    const expiresAt = moment(issuedAt).add(...parsedValue).valueOf()
    return expiresAt
}
...
export const click = async<T>(
    ...
    const day = new Date().toLocaleDateString('ru-RU', { weekday: 'long'})
...
export const transformToDate = (limit: string) => {
    ...
    const regex = /(d)|(w)|(y)/g
    const period = limit.match(regex)?.filter(
        (el) => el !== undefined
    ) as unknown as moment.unitOfTime.DurationConstructor
    const date = moment(new Date()).add(-+int + 1, period).toDate().toDateString()
    return date
}
...
export const generateOrderCode = (length: Number = 8) => {
    ...
    const alphabetLower = new Array(26).fill(1).map((_,i) => String.fromCharCode(97 + i))

    const alphabetUpper = new Array(26).fill(1).map((_, i ) => String.fromCharCode(65 + i))

    const numbers = new Array(10).fill(1).map((el, i) => i++)

    const alphaNumericAlphabet = [...alphabetLower, ...alphabetUpper, ...numbers]

    for(let i = 0; i <= length; i++){
        code += alphaNumericAlphabet[Math.floor(Math.random() * alphaNumericAlphabet.length)]
    }
...
export const calculateTotalClicks = (clickArray: clicks) => {
    if(clickArray.length > 0 ){
        return clickArray.flat().map((el) => el.count).reduce((a, b) => a + b)
    }else{
        return 0
    }
}